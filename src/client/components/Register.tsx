...
import { Mail, Lock, User, AlertCircle, Loader2, Sun, Moon } from 'lucide-react';
...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await financeApi.seedDefaults().catch(() => {});
      navigate('/');
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <TextInput type="text" value={fullName} onChange={e => setFullName(e.target.value)}
                placeholder="Jane Doe" leftIcon={<User className="w-3.5 h-3.5" />} />
            </div>
          ...
          <div className="mt-8 pt-8 border-t border-ink-100 dark:border-white/[0.06] text-center">
            <p className="text-sm text-ink-500 dark:text-white/40 font-light mb-3">Already have an account?</p>
            <Link to="/login"
